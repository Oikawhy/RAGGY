...
8. **Нет human escalation.** В legal-domain часть вопросов должна уходить юристу, особенно если есть риск, конфликт источников или недостаток фактов.
...
Planner Agent разбивает сложный юридический вопрос на подзадачи.
...
Он нужен потому, что юридические вопросы часто сформулированы бытовым языком, а документы написаны юридическим или англоязычным стилем.
...
В v1 high-risk запросы должны возвращать informational fallback или отправляться на human review.
...
- система возвращает fallback или human review.
...
- `low`: данных недостаточно, есть fallback, есть unsupported claims, нужен human review.

## 13. Observability и audit

Для юридического домена trace — это не только debugging, но и compliance artifact.
...
- где был fallback;
- кто approve-нул ответ, если был human review.
